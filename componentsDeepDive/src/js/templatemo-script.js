...
$(document).ready(function () {
    ...
    function goToPage(n) {
        ...
        // keep same behavior as before but use page- prefix
        filterValue = exclusives.length ? exclusives.join('') : '*';

        // add safe page class
        var wordPage = currentPage.toString();
        filterValue += ('.page-' + wordPage);
    ...
    function setPagination() {
        ...
        var SettingsPagesOnItems = function () {
            ...
            $container.children(filterValue).each(function () {
                ...
                // safely get classes, remove empty entries
                var classAttr = $(this).attr('class') || '';
                var classes = classAttr.split(/\s+/).filter(function (c) { return !!c; });

                // remove any existing page-* class
                classes = classes.filter(function (c) {
                    return !/^page-\d+$/.test(c);
                });

                // add the new page-* class
                classes.push('page-' + wordPage);

                // update classes without wiping everything else
